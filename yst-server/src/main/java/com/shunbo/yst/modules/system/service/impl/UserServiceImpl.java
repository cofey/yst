...
import com.shunbo.yst.component.ExcelExportComponent;
import com.shunbo.yst.component.ExcelImportComponent;
...
import com.shunbo.yst.modules.system.vo.UserExcelRow;
...
/**
 * UserServiceImpl 服务实现，负责执行业务流程与数据编排。
 */
@Service
...
public class UserServiceImpl implements UserService {
    ...
    private static final Map<String, SFunction<SysUser, ?>> LIST_SORT_COLUMNS = buildSortColumns();
    ...
    private final ExcelImportComponent excelImportComponent;
    private final ExcelExportComponent excelExportComponent;
    ...
    public void exportExcel(HttpServletResponse response) {
        ...
        excelExportComponent.writeRows(response, "用户列表", "用户", UserExcelRow.class, rows, "用户导出");
    }

    private void checkUsernameUnique(String username) {
        SysUser user = sysUserMapper.selectOne(new LambdaQueryWrapper<SysUser>()
                .eq(SysUser::getUsername, username));
        if (Objects.nonNull(user) && StringUtils.hasText(user.getUserId())) {
    ...
    private void saveUserCompanies(String userId, List<String> companyIds) {
        relationRebuildComponent.rebuild(companyIds,
    ...
    private static Map<String, SFunction<SysUser, ?>> buildSortColumns() {
        Map<String, SFunction<SysUser, ?>> sortColumns = new HashMap<>();
        sortColumns.put("username", SysUser::getUsername);
        sortColumns.put("nickname", SysUser::getNickname);
        sortColumns.put("status", SysUser::getStatus);
        sortColumns.put("createTime", SysUser::getCreateTime);
        sortColumns.put("create_time", SysUser::getCreateTime);
        return sortColumns;
    }
}
